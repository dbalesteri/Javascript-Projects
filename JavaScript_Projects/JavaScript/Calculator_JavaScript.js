...
const Calculator = {
    ...
    Display_Value : "0",
    //this holds the first operand for any expressions, set to null first
    First_Operand: null,
    //this checks whether or not the second operand has been input
    Wait_Second_Operand: false,
    //this holds the operator, set to null first
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    //this is checking to see if Wait_Second_Operand is true and set,
...
function Input_Decimal(dot) {
    //ensures that accidental clicking of decimal point doesn't cause bugs in the operation
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //this says that if Display_Value does not contain a decimal point, the decimal point should be then added
